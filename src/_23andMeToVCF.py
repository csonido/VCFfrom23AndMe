def from_file(input, output, fasta, fai):
    ...
    fai = __load_fai__(args)
    snps = __load_23andme_data__(args.input)
    records = __get_vcf_records__(snps, fai, args)
    __write_vcf__(args.output, records)

def __load_fai__(args):
    index = {}
...
def __get_vcf_records__(pos_list, fai, args):
